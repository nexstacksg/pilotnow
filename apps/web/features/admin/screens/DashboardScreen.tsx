  ...
  return (
    <div className="pn-stack">
      <div className="pn-stats">
        ...
        <StatCard icon={<TargetIcon size={16} stroke="#1F4FA3" strokeWidth={2} />} label="Ongoing jobs" value={metrics.ongoingJobs} hint="officers on duty now" tone="info" />
        <StatCard icon={<CameraOffIcon size={16} stroke="#FF3B30" strokeWidth={2} />} label="Missing hourly photos" value={metrics.missingPhotos} hint="needs follow-up" tone="danger" />
      </div>
        ...
        <div className="pn-stack">
          <Card>
            ...
            <div className="pn-action-grid">
              ...
              <button onClick={() => setScreen('billing')} type="button">
                <PaymentIcon size={18} stroke="#0A0A0A" strokeWidth={2} />
            ...
            {snapshot.unbilledJobs.length ? (
              snapshot.unbilledJobs.map((job) => (
                <button className="pn-list-row" key={job.id} onClick={() => openJob(job.id)} type="button">
                  ...
                  <span className="pn-muted-action">Open job</span>
                </button>
              ))
            ) : (
